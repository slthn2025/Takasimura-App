...
import com.example.takasimura.repository.ExpanseRepository
import kotlinx.coroutines.launch
import retrofit2.Response

class ExpanseViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val repository = ExpanseRepository(application.applicationContext)

    // Fungsi untuk mendaftar user
    fun expanse(date: String, amount:Int, description:String, wallet:String, category:String) {
        // Menjalankan operasi di dalam coroutine untuk melakukan pendaftaran secara asinkron
        viewModelScope.launch {
            try {
                // Panggil registerUser di repository
                val response: Response<ResponseExpanse> = repository.expanse(date, amount, wallet, description, category)
                ...
                if (response.isSuccessful) {
                    ...
                    registrationStatus.value = "Expanse Berhasil di tambahkan Kedalam Wallet: $message"
                } else {
